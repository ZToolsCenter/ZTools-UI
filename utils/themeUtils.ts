 ...
 * 应用根节点系统
 */
function detectOsPlatform(): OsPlatform {
  if (typeof window !== 'undefined' && window.ztools) {
    ...
    if (window.ztools.isLinux?.()) {
      return 'linux'
    }
  }

  if (typeof ztools !== 'undefined') {
    if (ztools.isMacOs?.() || ztools.isMacOS?.()) {
      return 'darwin'
    }
...
export function applyOsClass(platform?: OsPlatform): void {
  if (typeof document === 'undefined') {
    return
  }

  const resolvedPlatform = platform ?? detectOsPlatform()
